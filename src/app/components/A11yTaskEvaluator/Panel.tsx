...
export default function Panel({ title, result, onClose }: PanelProps) {
  return (
            ...
            {result.findings.length === 0 ? (
              ...
              result.findings.map((f, idx) => (
                <div key={idx} style={styles.finding}>
                  <div style={{ fontWeight: 800 }}>{f.kind.toUpperCase()}</div>
                  <div style={{ opacity: 0.92, marginTop: 2 }}>{f.message}</div>
